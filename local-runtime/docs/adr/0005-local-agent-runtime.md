...
- **Data:** 2026-09-13
- **Atualizado:** 2026-09-15 — `source=external` (BYO); MCP via catálogo HTTP
  ([ADR 0014](0014-mcp-host-isolated-http-registration.md)); stdio só nos pods `mcp-*`
- **Decisores:** Mantenedores da plataforma
- **Relacionada a:**
  [ADR 0003 — LiteLLM](0003-litellm-as-llm-gateway.md),
  [ADR 0004 — Local MCP Runtime](0004-local-mcp-runtime.md) *(histórico; ver 0014)*,
...
modelo. O host MCP local ([ADR 0014](0014-mcp-host-isolated-http-registration.md))
expõe `POST /mcp` por serviço `TEMPLATE=`; o catálogo Loom só vê HTTP.
...
- MCP local = URLs `streamable_http` no catálogo (pods `mcp-*`); AgentCore
  na AWS continua sem hosts locais no deploy;
...
Local Agent Runtime          ← loop do agente, cancel, isolamento
        ...
        └── MCP HTTP do catálogo (`mcp-*` / remoto)
```
...
5. **Tools só via MCP do payload.** URLs `streamable_http` do catálogo
   (`http://mcp-*:8787/mcp`). Sem SDK Azure/GitHub no agent-runtime.
6. **Authz no control plane; runtime fail-closed.** O backend já filtrou
   connectors e `allowed_tools`. O runtime respeita allowlist; enrich
   `service_bearer` (`MCP_RUNTIME_TOKEN`). O filho MCP não recebe JWT.
7. **SSE compatível com o Chat.** `session_start`, `chunk`, `session_end`,
   `error` — a UI não bifurca por `source=external`.
...
flowchart TB
  ...
  subgraph loom["Loom - stack local"]
    ...
    mr["mcp-* hosts<br/>TEMPLATE=/mcp<br/>streamable_http"]
    child["Filho MCP<br/>npx / uvx / …<br/>dentro do pod"]
  ...
  ac{{"AgentCore / Harness AWS<br/>data plane producao<br/>sem hosts locais no deploy"}}
  ...
  be -->|"valida token"| idp
  be --> db
  be -->|"source=external<br/>invoke + MCP payload"| ar
  ...
  mr -->|"stdio no pod"| child
  child --> azdo
  ac -->|"provider=litellm"| litellm
```

Fluxo de um invoke local com connector HTTP (o Chat não fala com o
...
sequenceDiagram
    ...
    BE->>BE: monta mcp_servers[] + enrich service_bearer
    ...
    loop tool loop
        ...
        alt tool MCP
            AR->>MCP: tools/call + service token
            MCP-->>AR: envelope sem secret
        end
        AR-->>BE: SSE chunk
        BE-->>FE: SSE chunk
    end
...
| `identity` | `subject`, `agent_id`, `session_id` (headers outbound MCP) |
| `approval_policies` | opcional; M3 |

Autenticação backend → agent-runtime: token de serviço (padrão
`AGENT_RUNTIME_TOKEN`), não o JWT do usuário.
...
| **M1** | `agent-runtime` + LiteLLM + MCP | Orientador + Azure DevOps HTTP no Chat; crash do agente não derruba o FastAPI |
...
- Agentes `source=external` (BYO) no compose.
- AgentCore/harness na AWS inalterados; hosts `mcp-*` não vão no deploy.
...
- BFF: `backend/app/services/local_invoke.py` faz proxy SSE quando
  ...
  vazio).
- MCP locais = catálogo `streamable_http` → `http://mcp-*:8787/mcp`
  ([ADR 0014](0014-mcp-host-isolated-http-registration.md)); sem
  `ensure_stdio_*` no BFF.
...
| 3 | Local Agent Runtime com contrato de invoke espelhado | **Escolhida.** Data plane local; control plane fino; reusa hosts `mcp-*` HTTP e LiteLLM. |
...
| 7 | Segundo catálogo de agentes “local-only” | **Rejeitada.** `source=external` no catálogo atual basta. |
...
- Chat com connectors locais passa a ter efeito (MCP real via URLs HTTP).
...
- Permitir hosts `mcp-*` em snapshots de deploy AgentCore “porque local já tem”.
